...
class _WeekPickerWidgetState extends State<WeekPickerWidget> with SingleTickerProviderStateMixin {
    ...
    return Column(
      ...
      children: [
        ...
        GridView.count(
          ...
          children: List.generate(52, (index) => _item("W${(index+1)}", index)),
        ),
        SizedBox(height: kPadding32),
  ...
  Widget _item(String quarter, int index) {
  ...
  List<DateTime> _getDates(String selected) {
    int week = (int.tryParse(selected.replaceAll("W", "")) ?? -1);
    if(week > 0) {
      int index = week - 1;
      DateTime startingDate = DateTime(_pickerYear, 1, 1);
      DateTime endingDate = DateTime(_pickerYear, 1, 7);
      DateTime startDate = startingDate.add(Duration(days: (7 * index)));
      DateTime endDate = endingDate.add(Duration(days: (7 * index) + (index == 51 ? 1: 0)));
      return [startDate, endDate];
    }
